...
ONEOCCURRENCE;
	...
	public function calupdate__NOTUSED() {
		...
		foreach ($events as $row) {
			...
			$eventsOptions .= "<OPTION VALUE='$id'>$name &nbsp; | &nbsp; {$row->mainCategory->inReverse()->one()->title}";
			if ($row->expiryDate > '0000-00-00') {
				$eventsOptions .= " &nbsp; | &nbsp; " . substr($row->startDate,0,10) . " - " . substr($row->expiryDate,0,10);
			}
			$eventsOptions .= "</OPTION>\n					";
	...
	<form method="post" action="" accept-charset="UTF-8">
		<input type="hidden" name="action" value="drycalendar/calupdate/AddAndDeleteInstances">
